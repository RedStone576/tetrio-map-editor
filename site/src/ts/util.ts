...
function pack(mapString: string)
{
...
function __compress(str: string)
{
    let string = ""
    let count  = 1
    ...
    for (let i = 1; i <= str.length; i++) 
    {
        ...
        {
            if (count > 1) string += count + str[i - 1]!
            else string += str[i - 1] 
...
function unpack(mapString: string)
{
    const parts = mapString.split("?")
...
function __decompress(str: string) 
{
    let string = ""
...
// very loose
function compare(mapString1: string, mapString2: string)
{
